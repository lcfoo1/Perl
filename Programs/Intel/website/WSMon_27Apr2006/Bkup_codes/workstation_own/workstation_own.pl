...
sub MainProgram
{
	my $ConfigFile = 'workstation_own.cfg';
	my $LogFile = 'C:\Intel\Perl\Programs\workstation\SummaryWS.tmp';	
	my $OkLogFile = 'C:\Intel\Perl\Programs\workstation\SummaryWS.txt';
	my $pgxpw4035 = "\\\\pgxpw4035\\c\$\\intel\\Perl\\Programs\\SummaryWS_pgxpw4035.txt";
	my $FailPingFlag = 0;
	...
	while (<CONFIG>)
	{
		...
		{
			chomp($Ping);
			if ($Ping =~ /Request\s*timed\s*out\./iog)
			{
				$FailPingFlag = 1;
			}
		}
		...
		foreach my $Process ($ProcessInfo->ListPids()) 
		{
			...
			foreach my $Line (sort keys %$Process)
			{
				...
				{
					if (($Process->{'Owner'} =~ /^\w{3}\\(\w+)/) && ($LogonFlag))
					{
						$IDSIDs = $1;
	...
	copy($OkLogFile, $pgxpw4035) || die "Cant copy $OkLogFile : $!\n";
}
