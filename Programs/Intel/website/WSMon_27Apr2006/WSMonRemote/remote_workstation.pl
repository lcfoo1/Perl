...
# ServerSetup=1 for running at Server, ServerSetup=0 for running at remote
my $ServerSetup = 0;
my $CurrentPath = 'C:/winnt/WSMon/';
my $RemotePath = $CurrentPath . 'WSMonRemote/';
my $Server = 'pgxpw4035';
...
sub MainProgram
{
	my $ServerLog = "\\\\". $Server . "\\c\$\\winnt\\WSMon\\SummaryWS_" . $Server . ".txt";
	...
	if ($ServerSetup == 0)
	{	
		$ConfigFile = $RemotePath . 'remote_workstation.cfg';
		$LogFile = $RemotePath . 'remote_SummaryWS.tmp';
		$OkLogFile = $RemotePath . 'remote_SummaryWS.txt';
	}
	...
	while (<CONFIG>)
	{
		...
		foreach my $Ping (@Pings)
		{
			...
			else
			{
				...
				if ((!$qwinsta) && (($Server =~ /pgw/ig) || ($Server =~ /pgxpvmu/ig)))
				{
					$qwinsta = 0;
	...
	if ($ServerSetup == 0)
	{
		copy($OkLogFile, $ServerLog) || die "Cant copy $OkLogFile to $ServerLog : $!\n";
	}
