...
sub Main
{
	...
	while (<FILE>)
	{
		...
		elsif ((/(.*\{\s*V\s*\{\s*ALLPINs=)(.*)/i) || (/(.*\{\s*V\s*\{\s*all_sideband_pins=)(.*)/i))
		{
			...
			# Getting the pattern address from CMT .pat.data
			$_ = <FILE>;
			if ((/(.*\s*W\s*\{\s*ALLPINs=)(.*)/i) || (/(.*\s*W\s*\{\s*all_sideband_pins=)(.*)/i))
			{
				my ($AllPinsParam2, $Bit2s) = ($1, $2);
				chomp ($Bit2s);
				$Bit2s =~ s/;//;
				$Bit2s =~ s/\}//;
				my @Addr = split(/#/,$Bit2s);
			...
			if ($Address eq "NA")
			{
				print "Failed to run due to invalid address format in .pat.data at line $.\n";
				exit 0;
			}

			for (my $i = 0; $i <= $#FoundPins; $i++)
			{
				$Tmp[$i] = $DataCol[$AllPinsPos[$i] - $LenAllPinsParam];
				no warnings;
				if ($PostTmp[$i] eq $Tmp[$i])
				{
				...
				{
					print "$FoundPins[$i] - $DataCol[$AllPinsPos[$i] - $LenAllPinsParam] $Address\n";
				}
