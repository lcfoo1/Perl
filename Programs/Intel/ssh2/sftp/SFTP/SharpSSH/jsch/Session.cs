...
namespace SharpSsh.jsch
{
	...
	{
		private static String version="SharpSSH-"+SharpSsh.SshBase.Version.ToString()+"-JSCH-0.1.28";
		...
		public void connect(int connectTimeout) 
		{
			...
			try	
			{
				...
				V_S=new byte[i]; SharpSsh.java.System.arraycopy(buf.buffer, 0, V_S, 0, i);
				...
				//io.put(V_C, 0, V_C.Length); io.put("\n".getBytes(), 0, 1);
			{
				...
				SharpSsh.java.System.arraycopy(V_C, 0, foo, 0, V_C.Length);
		...
		private KeyExchange receive_kexinit(Buffer buf)  
		{
			...
			SharpSsh.java.System.arraycopy(buf.buffer, buf.s, I_S, 0, I_S.Length);
			/*
			try{
			byte[] tmp=new byte[I_S.Length];
			SharpSsh.java.System.arraycopy(I_S, 0, tmp, 0, I_S.Length);
...
//					((buf.buffer[3]    )&0x000000ff);
		...
		private void updateKeys(KeyExchange kex) 
		{
			...
			if(session_id==null)
			{
				...
				SharpSsh.java.System.arraycopy(H, 0, session_id, 0, H.Length);
			}
			...
			*/
			...
			try
			{
				...
				while(s2ccipher.getBlockSize()>Es2c.Length)
				{
					...
					SharpSsh.java.System.arraycopy(Es2c, 0, bar, 0, Es2c.Length);
					SharpSsh.java.System.arraycopy(foo, 0, bar, Es2c.Length, foo.Length);
					Es2c=bar;
				}
				...
				while(c2scipher.getBlockSize()>Ec2s.Length)
				{
					...
					SharpSsh.java.System.arraycopy(Ec2s, 0, bar, 0, Ec2s.Length);
					SharpSsh.java.System.arraycopy(foo, 0, bar, Ec2s.Length, foo.Length);
					Ec2s=bar;
				}
