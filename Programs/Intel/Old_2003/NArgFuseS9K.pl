...
#                                                                               #
#        DESCRIPTION                                                            #
#        Runs through all production iTUFF's and detects unfused units. If      #
#        found sends an alert email.                                            #
#                                                                               #
#        NOTES                                                                  # 
#        Runs as a cronjob every 30 minutes on t3admin6.png.intel.com           #
#                                                                               #
#        RELEASES                                                               #
#        07/26/2004  rev1.0 - Main code release                                 #
#                                                                               #
#################################################################################
...
my (@LotDir, @TmpBadBin, %DevicePEList, %ListLotError, %BadBin2, %BadBin1);
my ($TmpReportHTML, $CommonEmailList, $ErrorBinFlag, $FoundLotFlag) = ("", "", 0, 0);
my $Now = &FormatNow();
my $ProdDir = $ARGV[0];
...
# Record no lot tested at log file, to ensure that the cronjob is running
sub RecordLog
{
...
sub GetProdTable
{
	...
	if ($CommonEmailList eq "")
	{
		...
		my $To = "To: $TmpTo";
		my $Subject = "Subject: Email table in wrong format at $Now";
...
sub CheckFuse
{
	...
	open (ITUFF, $Summary) or die "Can't open $Summary : $!";
	while (<ITUFF>)
	{
		s/ //g;
		$PrtName = $1 if /^3_prtnm_(\d+)/o;
		...
		{ 
			...
			# Tester shown bin 2 fusing but fused as bin 1
		...
		{ 
			...
			# Tester shows bin 1 fusing but fused as bin 2
...
# Rearrange the list from 1,2,3 to 1-3 - Theres a module for this!
