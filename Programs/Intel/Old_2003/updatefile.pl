...
#	If exist duplication, return boolean DUP, else return boolean NODUP		#
# 	to SACFTPToIPED.pl								#
#											#
...
my ($productid, $dataxy, $WWID, $StartDate, $Time, $EndDate, $devrv, $progname, $productid_devrv);
my (@newcdup, @data, @KeyProd, @cdup);
my @xloc=0;
my @yloc=0;
...
# Sending duplicate, mismatch x-y min and max coordinate, wrongly inserted Fabrun Boolean to SACFTPToIPED.pl
	if (($dup eq "DUP") && ($match eq "MATCH") && ($fab eq "FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch MATCH FABRUN 0 0 DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "NOT_MATCH") && ($fab eq  "FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch NOT_MATCH FABRUN $actual_minmax $ITUFF_minmax NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch MATCH WRONG_FABRUN 0 0 NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "NOT_MATCH") && ($fab eq "FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch NOT_MATCH FABRUN $actual_minmax $ITUFF_minmax DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	} 
	elsif (($dup eq "DUP") && ($match eq "MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch MATCH WRONG_FABRUN 0 0 DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "NOT_MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch NOT_MATCH WRONG_FABRUN $actual_minmax $ITUFF_minmax NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "NOT_MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgMatch"))
	{
		print "$File ProgMatch NOT_MATCH WRONG_FABRUN $actual_minmax $ITUFF_minmax DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "MATCH") && ($fab eq "FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch MATCH FABRUN 0 0 DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "NOT_MATCH") && ($fab eq  "FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch NOT_MATCH FABRUN $actual_minmax $ITUFF_minmax NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch MATCH WRONG_FABRUN 0 0 NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "NOT_MATCH") && ($fab eq "FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch NOT_MATCH FABRUN $actual_minmax $ITUFF_minmax DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch MATCH WRONG_FABRUN 0 0 DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	elsif (($dup eq "NODUP") && ($match eq "NOT_MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch NOT_MATCH WRONG_FABRUN $actual_minmax $ITUFF_minmax NODUP\n";
		&RemoveTemp;
	}
	elsif (($dup eq "DUP") && ($match eq "NOT_MATCH") && ($fab eq "WRONG_FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch NOT_MATCH WRONG_FABRUN $actual_minmax $ITUFF_minmax DUP $dataxy $WWID $StartDate $Time $EndDate $productid_devrv\n";
		&RemoveTemp;
	}
	
	elsif (($dup eq "NODUP") && ($match eq "MATCH") && ($fab eq "FABRUN") && ($ProgramMatch eq "ProgNotMatch"))
	{
		print "$File ProgNotMatch MATCH FABRUN 0 0 NODUP\n";
		&RemoveTemp;
	}
	else
	{
		print "$File ProgMatch MATCH FABRUN 0 0 NODUP\n";    
		&RemoveTemp;
	}
