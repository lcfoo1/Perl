...
my $Int1 = $Column1 -> Entry (-width => 10) -> pack;
$Int1->insert('end',0);
...
my $Int2 = $Column2 -> Entry(-width=>10)->pack;
$Int2->insert('end',0);
...
sub Calculate
{
	...
	my $TmpDivInt = $Integer1 / $Integer2;	 
	
	$DivInt = $1 if ($TmpDivInt =~ /^(\d+\.\d{1,3})/);

	print "Add: $AddInt, Minus: $MinusInt, Multiple: $MultiInt, Divide: $DivInt\n\a";
	
	&Invalid if (($Integer1 !~ /^\d{1,3}$/) || ($Integer2 !~/^\d{1,3}$/));
}
...
sub Invalid
{
	...
	$MultiInt = "-";
	$DivInt = "-";
	my $Msg = $main ->Toplevel (-title=>'Warning');
	$Msg->Label (-font=>30, -background=>'green',-text=>'Invalid data')->pack;
