...
$TmpFile = $1 if ($WaferDataFile =~ /tmp\/(\S+)$/o);
...
sub ProcessToiTUFF
{
	($WaferIDTokenFlag,  $ErrorFlag) = (0, 0);
	$ErrorLine = "";
	...
	while(<WAFERDATA>)
	{
		($TestName, $Result, $FDPMV, $Vcont, $FailingPattern, $Faildata, $xloc, $yloc, $Bin, $TestTime, $xyloc) = ();
		...
		if ($FoundUnitFlag)
		{
			...
			if ($Store_xy{$xyloc} eq "")
			{
				$Store_xy{$xyloc} = $xyloc;
			...
			{
				# Don't insert x-y coordinate to avoid duplicate. Just insert the first detected x-y location
				print "Found duplicate at (x, y) = ($xloc, $yloc)\n";
				$ErrorLine .= "(x, y) = ($xloc, $yloc) - duplicate\n";
				$ErrorFlag = 1;
	...
	$ArchiveFile = "/lopte/data/tmp/inkless_archive/" . $TmpFile;
	qx/mv $WaferDataFile $ArchiveFile/;

	if($ErrorFlag)
	{
		$ErrorLog = "/lopte/data/tmp/inkless_archive/log/" . $TmpFile;
		open (ERROR, ">>$ErrorLog") || die "Cannt open file $ErrorLog : $!";
...
sub SendMail
{
        @Tos = ('lye.cheung.foo@intel.com', 'ching.tatt.teoh@intel.com'); 
        $ToS = join (",", @Tos); 
        $To = "To: $ToS\n";
        $From = "From: t3hp94k5\@png.intel.com\n";
        $Subject = "Subject: Found Error \n";
        $User = qx/whoami/;
        $Content = <<"MAILBODY";
...
MAILBODY
        ...
        close SENDMAIL || warn "Didnt close as expected:- $!\n";
}
